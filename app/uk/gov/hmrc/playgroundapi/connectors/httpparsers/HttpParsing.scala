...
import uk.gov.hmrc.playgroundapi.models.{DownstreamError, MultipleDownstreamErrors, ServiceError, SingleDownstreamError}
...
trait HttpParsing {
  ...
  def parseErrors(response: HttpResponse): DownstreamError = {
    val singleError: Option[MultipleDownstreamErrors] = response.validateJson[SingleDownstreamError].map(err => MultipleDownstreamErrors(List(err)))
    lazy val multipleErrors = response.validateJson[MultipleDownstreamErrors]
    lazy val unableToParseJsonError: ServiceError = {
